...
def load_templates(script_dir):
    ...
    jpg_dir = os.path.join(script_dir, "jpg19")
    if not os.path.exists(jpg_dir):
        print(f"未找到 jpg19 目录: {jpg_dir}")
        return []
...
def main():
    # 1. 获取三层窗口句柄
    # 最外层窗口：LDPlayerMainFrame / 国家攻城4队
    hwnd_outer = get_window_handle("LDPlayerMainFrame", "国家攻城4队")
    if hwnd_outer == 0:
        return
