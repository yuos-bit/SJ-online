...
def load_templates(script_dir):
    jpg_dir = os.path.join(script_dir, "jpg-3")  # 按顺序读取 jpg-3 下的图片
    if not os.path.exists(jpg_dir):
        print(f"未找到 jpg-3 目录: {jpg_dir}")
        return []
    jpg_files = sorted(
...
def main():
    ...
    MAX_ROUNDS = 90         # 最大轮数（1-8 全部完成算 1 轮）
    ...
    while rounds < MAX_ROUNDS:
        ...
        if pt and score >= CLICK_THRESHOLD:
            ...
            time.sleep(0.4)  # 点击后小延迟
        else:
            time.sleep(0.01)  # 未找到时快速轮询
